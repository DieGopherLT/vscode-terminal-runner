...
// handleBridgeError processes and formats security-specific errors for better user experience.
...
func handleBridgeError(err error) error {
	...
	case strings.Contains(errMsg, "no valid bridge found"):
		styles.PrintError("No valid bridge instances found.")
	...
	default:
		styles.PrintError(fmt.Sprintf("Bridge error: %v", err))
		return err
	}
}
