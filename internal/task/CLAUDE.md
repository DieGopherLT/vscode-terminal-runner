...
**Submission flow:** Enter on Submit -> `handleTaskCreation` -> `isValidTask` (in-memory checks only, errors to `MessageManager`) -> if valid, `submitTaskCmd` runs async: `validateTaskPath` (`os.Stat`) -> reject name collision for create/rename via `FindByName` (case-sensitive; same-name edits exempt) -> `saveTask` -> `taskSaveResultMsg` -> `tea.Quit`.
